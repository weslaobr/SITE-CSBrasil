        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-4 max-w-2xl">
            ...
            <div className="flex items-center gap-4 bg-black/40 p-2 pr-6 rounded-full w-fit border border-white/5">
              {list.creator?.image ? (
                <img src={list.creator.image} className="w-8 h-8 rounded-full border border-white/10" alt={list.creator.name} />
              ) : (
              ...
              <div>
                ...
                <p className="text-xs text-white font-black">{list.creator?.name || "Anônimo"}</p>
              </div>
            </div>
          </div>
