                        ...
                        {serverControlTab === 'maps' && (
                            <div className="p-5">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                    {maps.map(map => (
                                        <div key={map.id} className={`group relative aspect-video rounded-xl border overflow-hidden transition-all ${map.active ? 'border-white/10 bg-white/5' : 'border-white/5 bg-black/20 opacity-40 grayscale'}`}>
                                            <img src={map.image} alt={map.name} className="object-cover w-full h-full" />
                                            ...
                                            <button 
                                                ...
                                                className={`absolute top-1.5 right-1.5 p-1 rounded-lg border backdrop-blur-md z-30 transition-all ${
                                            ...
                                            {/* Map Info (Visible Always) */}
                                            <div className="absolute bottom-1.5 left-2 right-2">
                                                <p className="text-[9px] font-black uppercase text-white truncate">{map.name}</p>
                                                <p className="text-[7px] text-zinc-500 font-mono truncate">{map.id}</p>
                                            </div>

                                            {/* Change Map Button (Visible on Hover) */}
                                            <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center p-3">
                                                <button 
                                                    onClick={() => {
                                                        if (confirm(`Trocar para ${map.name} agora?`)) {
                                                            let cmd = `map ${map.id}`;
                                                            if (map.id.startsWith('workshop/')) {
                                                                const parts = map.id.split('/');
                                                                const workshopId = parts[1];
                                                                cmd = `host_workshop_map ${workshopId}`;
                                                            }
                                                            setLogs(prev => [...prev.slice(-150), `>>> Solicitando troca: ${cmd}`]);
                                                            sendCommandRaw(cmd);
                                                        }
                                                    }}
                                                    disabled={!map.active}
                                                    className="w-full py-1.5 bg-yellow-500 text-black text-[9px] font-black uppercase rounded-lg hover:bg-yellow-400 transition-all active:scale-95 disabled:opacity-50"
                                                >
                        ...
                        {serverControlTab === 'controls' && (
                            <div className="p-5 space-y-4">
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        { label: '🔄 Reiniciar Round',   cmd: 'mp_restartgame 1',           cls: 'yellow', desc: 'Reinicia o round atual imediatamente.' },
                                        { label: '⏸️ Pausar Partida',    cmd: 'mp_pause_match',             cls: 'blue',   desc: 'Pausa a partida no próximo freezetime.' },
                                        { label: '▶️ Despausar',         cmd: 'mp_unpause_match',           cls: 'green',  desc: 'Retoma a partida pausada.' },
                                        { label: '🌡️ Iniciar Warmup',    cmd: 'mp_warmup_start',            cls: 'orange', desc: 'Inicia o período de aquecimento.' },
                                        { label: '⏭️ Pular Warmup',      cmd: 'mp_warmup_end',              cls: 'orange', desc: 'Encerra o aquecimento e inicia a partida.' },
                                        { label: '🤖 Limpar Bots',       cmd: 'bot_kick',                   cls: 'red',    desc: 'Remove todos os bots do servidor.' },
                                        { label: '🔪 Faca apenas',       cmd: 'mp_give_player_c4 0; mp_buy_allow_knives 1; mp_startmoney 0; mp_restartgame 1', cls: 'purple', desc: 'Configura o round para ser apenas de faca.' },
                                        { label: '💰 Reset Dinheiro',    cmd: 'mp_startmoney 16000; mp_restartgame 1', cls: 'cyan', desc: 'Define o dinheiro inicial para $16.000 e reinicia.' },
                                        { label: '💡 God Mode',          cmd: 'sv_cheats 1; god',           cls: 'pink',   desc: 'Ativa cheats e o modo invencível.' },
                                        { label: '🚫 Sem cheats',        cmd: 'sv_cheats 0',                cls: 'red',    desc: 'Desativa o uso de comandos de trapaça (sv_cheats 0).' },
                                        { label: '📢 Anunciar no chat',  cmd: '__custom_say__',             cls: 'zinc',   desc: 'Envia uma mensagem global no chat do servidor.' },
                                        { label: '🔁 Trocar de lado',    cmd: 'mp_swapteams; mp_restartgame 1', cls: 'yellow', desc: 'Inverte os times (CT <> TR) e reinicia.' },
                                        { label: '▶️ Forçar Início',    cmd: 'css_start',                  cls: 'green',  desc: 'Força o início da partida ignorando jogadores não prontos.' },
                                        { label: '⏪ Restaurar Round',  cmd: '__custom_restore__',         cls: 'blue',   desc: 'Restaura a partida para um round específico do backup.' },
                                        { label: '🔪 Round de Faca',    cmd: 'css_roundknife',             cls: 'purple', desc: 'Ativa ou desativa a execução do round de faca.' },
                                        { label: '⏭️ Pular Veto',       cmd: 'css_skipveto',               cls: 'orange', desc: 'Pula a fase de escolha/banimento de mapas.' },
                                        { label: '🤝 Fogo Amigo ON',    cmd: 'mp_friendlyfire 1',          cls: 'red',    desc: 'Ativa o dano entre colegas de equipe.' },
                                        { label: '🤝 Fogo Amigo OFF',   cmd: 'mp_friendlyfire 0',          cls: 'zinc',   desc: 'Desativa o dano entre colegas de equipe.' },
                                        { label: '🗣️ AllTalk ON',       cmd: 'sv_alltalk 1',               cls: 'blue',   desc: 'Permite que todos os jogadores se ouçam e falem entre si.' },
                                        { label: '🤫 AllTalk OFF',      cmd: 'sv_alltalk 0',               cls: 'zinc',   desc: 'Restringe a comunicação apenas para a própria equipe.' },
                                        { label: '🛠️ Modo Prática',     cmd: 'css_prac',                   cls: 'cyan',   desc: 'Ativa o modo de treino com utilitários infinitos.' },
                                        { label: '🚪 Sair do Treino',   cmd: 'css_exitprac',               cls: 'zinc',   desc: 'Desativa o modo de treino e volta ao normal.' },
                                        { label: '🌑 Limpar Granadas',  cmd: 'css_clear',                  cls: 'pink',   desc: 'Remove instantaneamente todas as smokes e molotovs.' },
                                        { label: '🔄 Refazer Granada',  cmd: 'css_rethrow',                cls: 'purple', desc: 'Joga novamente a última granada lançada por você.' },
