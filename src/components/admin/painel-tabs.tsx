...
import { Terminal as ConsoleIcon, Map as MapIcon, HardDrive as DemoIcon, Users as UsersIcon, Trophy as MatchesIcon, Activity as QueueIcon, FolderOpen as FilesIcon } from 'lucide-react';
...
import { FileManagerTab } from "@/components/admin/file-manager-tab";

export function PainelTabs() {
    const [activeTab, setActiveTab] = useState<'server' | 'maps' | 'demos' | 'users' | 'matches' | 'queue' | 'files'>('server');
            ...
            <div className="px-6 md:px-10 mt-6">
                <div className="flex items-center gap-1 bg-white/5 border border-white/5 p-1 rounded-2xl w-fit overflow-x-auto no-scrollbar max-w-full">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'files' ? 'bg-yellow-500 text-black shadow-lg shadow-yellow-500/10' : 'text-zinc-500 hover:text-white hover:bg-white/5'}`}
                    >
                        <FilesIcon size={14} />
                        Arquivos
                    </button>
